...
public class EmployeeExcelImportService {
            ...
            for(int index=header.getRowNum()+1;index<=sheet.getLastRowNum();index++){
                ...
                try{
                    ...
                    if(exists(employeeNumber)){
                        results.add(alreadyExists(rowNumber,employeeNumber));
                        continue;
                    }
                    ...
                    results.add(new EmployeeImportRowResult(rowNumber,true,saved.id(),employeeNumber,null,false,null));
                }catch(Exception exception){
                    if(exception instanceof ResponseStatusException response && response.getStatusCode().equals(HttpStatus.CONFLICT)
                            && employeeNumber!=null && exists(employeeNumber)){
                        results.add(alreadyExists(rowNumber,employeeNumber));
                    }else results.add(new EmployeeImportRowResult(rowNumber,false,null,employeeNumber,message(exception),false,null));
                }
            }
            int accepted=(int)results.stream().filter(EmployeeImportRowResult::accepted).count();
            int skipped=(int)results.stream().filter(EmployeeImportRowResult::skipped).count();
            return new EmployeeImportResult(results.size(),accepted,results.size()-accepted-skipped,skipped,List.copyOf(results));
    ...
    private UUID master(String table,String value){
        if(value==null)return null;
        String column=switch(table){
            case "designation" -> "title";
            case "branch","department","grade" -> "name";
            default -> throw new IllegalArgumentException("Unsupported employee master: "+table);
        };
        return db.sql("SELECT id FROM "+table+" WHERE tenant_id=:tenant AND UPPER("+column+")=UPPER(:name)")
                .param("tenant",TenantContext.requireTenantId()).param("name",value).query(UUID.class)
                .optional().orElseThrow(()->bad("Unknown "+table+": "+value));
    }
    ...
    private static EmployeeImportRowResult alreadyExists(int rowNumber,String employeeNumber){
        return new EmployeeImportRowResult(rowNumber,false,null,employeeNumber,null,true,"Already exists");
    }
